...
function optimizeLCP() {
  ...
  // 1. Optimizar el título principal en Home.jsx
  const homePath = path.join(__dirname, "src/pages/Home.jsx");
  if (fs.existsSync(homePath)) {
    let homeContent = fs.readFileSync(homePath, "utf8");

    // Reemplazar el título con optimizaciones LCP
    const optimizedTitle = `              {/* Título principal optimizado para LCP */}
              <motion.h1
                className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-bubbly text-white mb-6 font-bold glow-on-hover"
                initial={{ opacity: 0, y: 30 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6, ease: "easeOut" }}
                style={{
                  textShadow: "0 0 20px rgba(237, 1, 127, 0.5), 0 0 40px rgba(255, 183, 27, 0.3)",
                  // Optimizaciones LCP
    ...
    // Buscar y reemplazar el título existente
    const titleRegex =
      /<motion\.h1[\s\S]*?LA REVOLUCIÓN BELICONA[\s\S]*?<\/motion\.h1>/;
    homeContent = homeContent.replace(titleRegex, optimizedTitle);

    fs.writeFileSync(homePath, homeContent);
    console.log("✅ Título principal optimizado para LCP");
  }

  // 2. Crear CSS crítico optimizado para LCP
...
.preload-hint {
  ...
  // 3. Crear componente de preload para fuentes
...
export default FontPreloader;`;
  ...
  // 4. Optimizar index.html para LCP
...
${criticalLCPCSS}
  ...
  // 5. Crear script de medición LCP
...
(function() {
  ...
  console.log("1. ✅ Título principal optimizado con hardware acceleration");
  console.log("2. ✅ CSS crítico LCP inline");
  console.log("3. ✅ Preload de fuentes críticas");
  console.log("4. ✅ Preload de recursos críticos");
  console.log("5. ✅ Script de medición LCP");
